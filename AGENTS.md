# AGENTS.md

This repository is a small Python app for asset allocation configuration
and anti-FOMO portfolio monitoring. The guidance below is for agentic coding
tools working in this repo.

## Architecture

The project has two entry points:

1. **`run.py`** -- CLI for portfolio monitoring (single check or daily scheduler).
2. **`serve.py`** -- HTTP server for the Web-based asset configuration UI.

`serve.py` generates `config.asset.yaml`, which is the **sole source** of
portfolio holdings (stock names, codes, ratios). `config.yaml` only holds
operational settings (thresholds, AI analysis, data fetch, notification,
scheduler).

Core module dependency (simplified):

```
run.py -> src/main.py (AntiFOMO)
              |- src/portfolio.py        (Portfolio, Holding)
              |- src/data_fetcher.py     (DataFetcher -> akshare)
              |- src/volatility_calculator.py
              |- src/threshold_manager.py
              |- src/ai_analyzer.py      (AIAnalyzer -> OpenAI SDK)
              +- src/notification.py

serve.py -> web/ (static SPA) -> config.asset.yaml (generated)
```

Note: `run.py` inserts `src/` into `sys.path` at runtime, so local imports
within `src/` use bare module names (e.g., `from portfolio import Portfolio`).

## Quick Start Commands

```bash
pip install -r requirements.txt        # install deps
python serve.py                        # web UI at http://localhost:8080
python run.py                          # single volatility check
python run.py schedule                 # daily scheduler
```

## Build / Lint / Test

There is no formal build system, linter, or unit test suite yet.

- Build: N/A
- Lint: N/A
- Test: N/A

If you add tests, place them in a `tests/` folder and provide a way to run
a single test:

```bash
pytest tests/test_x.py::test_name
```

Keep tests deterministic -- mock network calls (`akshare`) and LLM APIs.

## Configuration Files

| File | Purpose |
|------|---------|
| `config.yaml` | Operational config (thresholds, AI, data fetch, notification, scheduler). No portfolio holdings. No secrets. |
| `config.asset.yaml` | Generated by `serve.py`. Sole source of holdings for `run.py`. Gitignored. |
| `config.example.yaml` | Example config for new users. |

Environment variables (preferred for secrets):
- `OPENAI_API_KEY` or `AI_API_KEY`
- `OPENAI_BASE_URL` (for compatible providers)

## Code Style Guidelines

### General

- Language: Python 3.x.
- **No emoji anywhere** -- logs, code, comments, commit messages, docs.
  Keep all output concise, professional, and clear.
- Favor small, readable functions over deep abstraction.
- Avoid unnecessary comments; explain only non-obvious behavior.

### Imports

- Standard library first, then third-party, then local imports.
- Keep imports at top of file (lazy imports in functions are acceptable when
  the dependency is heavy or optional, e.g., `import schedule`).
- Use explicit imports; no wildcards.

### Formatting

- Indentation: 4 spaces.
- Use double quotes for strings (matches current codebase).
- Keep line length reasonable (100-120 chars).
- Prefer f-strings for formatting.
- Do not auto-format or reorder YAML keys unless necessary.

### Types

- Use type hints for public functions and dataclass fields.
- Prefer `Optional[T]` for nullable values.
- Dataclasses (`@dataclass`) are the standard for data containers.
  Implement `__str__` on dataclasses when human-readable output is useful.

### Naming Conventions

- Classes: `CamelCase` (e.g., `AntiFOMO`, `DataFetcher`).
- Functions / variables: `snake_case`.
- Private helpers: prefix with `_` (e.g., `_load_config`).
- Constants: `UPPER_SNAKE_CASE` (e.g., `PORT`, `BASE_DIR`).
- Enums: `CamelCase` class, `UPPER_SNAKE_CASE` members
  (e.g., `AlertLevel.WARNING`, `HoldingType.INDEX`).
- File names: `snake_case.py`.

### Error Handling

- Prefer graceful failure with clear console messages.
- For external calls (network / API), wrap in try/except and return safe
  defaults (empty DataFrame, `None`) where appropriate.
- Use `raise SystemExit(1)` only for truly fatal config errors.
- Use `ValueError` only for programmer errors (invalid arguments).
- Avoid raising exceptions for user-supplied config unless the system
  cannot proceed; use error messages instead.

### IO and Side Effects

- Configuration parsing lives in `src/main.py` and
  `src/asset_configurator.py`.
- Network access is centralized in `src/data_fetcher.py`.
- Notification output is centralized in `src/notification.py`.
- Avoid writing files outside explicit config outputs.

### AI Analysis

- `src/ai_analyzer.py` is the only module that should call LLM APIs.
- Do not hardcode API keys; use environment variables or config.
- If adding providers, keep a consistent interface and config keys.

### Asset Configuration

- `serve.py` serves `web/` and provides the config generation API.
- `config.asset.yaml` is the sole source of holdings for `run.py`.
- Keep the web asset schema simple: name, code, amount, type, region,
  style.
- `src/asset_configurator.py` is the legacy interactive CLI wizard
  (superseded by `serve.py`). Keep it functional but do not expand it.

## Repository Conventions

- No Cursor or Copilot rule files are present.
- Keep `config.yaml` free of secrets and free of portfolio holdings.
- Documentation lives in two files only: `README.md` (user-facing) and
  `AGENTS.md` (agent guidance). Do not create additional top-level
  markdown docs.
- `docs/` is reserved for AI conversation memory / short-term notes
  between sessions. Do not put project documentation there.
- `web/` is a vanilla HTML/CSS/JS SPA with no build tools or frameworks.

## Suggested Manual Verification

```bash
python serve.py          # open browser, save assets
python run.py            # confirm single check completes
```
